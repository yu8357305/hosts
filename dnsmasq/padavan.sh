#!/bin/sh
#此脚本只适用于老毛子padavan固件
#初次运行： wget --no-check-certificate https://raw.githubusercontent.com/sy618/hosts/master/dnsmasq/padavan.sh -O /etc/storage/dnsmasq/padavan.sh;sh /etc/storage/dnsmasq/padavan.sh
echo "————————————开始脚本—————————————"
echo " "
echo "·········写入配置信息···········"
...
echo "--- 下载dnsmasq规则"
wget --no-check-certificate https://raw.githubusercontent.com/sy618/hosts/master/dnsmasq/dnsfq -O /etc/storage/dnsmasq/dns/dnsfq;sed -i "1 i\## Download：\$(date "+%Y-%m-%d %H:%M:%S")" /etc/storage/dnsmasq/dns/dnsfq
